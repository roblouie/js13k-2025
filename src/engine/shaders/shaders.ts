...
precision highp float;
in vec2 i;in float n;in vec3 o;in mat4 l;in vec4 e;uniform mediump sampler2DArray h;uniform mediump sampler2DShadow t;vec3 s=vec3(-1,1.5,-1);float z=.2f;vec2 A[5]=vec2[](vec2(0),vec2(-1,0),vec2(1,0),vec2(0,1),vec2(0,-1));float u=1.,C=4200.;out vec4 d;void main(){for(int v=0;v<5;v++){vec3 m=vec3(e.xy+A[v]/C,e.z-.001);float f=texture(t,m);u*=max(f,.87);}vec3 v=normalize(mat3(l)*o),f=normalize(s);float m=max(dot(f,v)*u,z);vec3 D=m*vec3(1);vec4 F=vec4(D.xyz,1);d=texture(h,vec3(i,n))*F;if(d.w<.5)discard;}`;

export const skybox_fragment_glsl = `#version 300 es
precision highp float;
uniform samplerCube D;uniform mat4 F;in vec4 g;out vec4 d;void main(){vec4 v=F*g;d=texture(D,v.xyz);}`;

export const skybox_vertex_glsl = `#version 300 es
layout(location=0) in vec4 J;out vec4 g;void main(){g=J;gl_Position=J;gl_Position.z=1.;}`;
